...
class GlobalMetrics:
    ...
    def print_summary(self):
        ...
        print("\n" + "="*80)
        print("📊 JOBHUNT AI SCRAPER SUMMARY V2.5")
        print("="*80)
...
class JobEngine:
    ...
    async def run(self):
        ...
        logger.info("="*80)
        logger.info("🚀 JOBHUNT AI - PRODUCTION SCRAPER V2.5 (FIXED & ANALYTICS)")
        logger.info("="*80)
        logger.info(f"📅 Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        logger.info(f"🎯 Targets: {len(TARGETS)} companies (sorted by priority)")
        logger.info(f"⚙️  Config: {15} max concurrency, {3} retries")
        ...
        async with aiohttp.ClientSession(
            ...
            logger.info(f"🔝 Processing {len(high_priority)} high-priority companies first...")
        ...
        try:
            ...
            for job in jobs:
                for profile in profiles:
                    ...
                    # 🆕 Handle explicit rejection for better logging/debugging
                    if scoring.get('rejected'):
                        # Optional: logger.debug(f"   ❌ Skipped {job['title']}: {scoring.get('rejection_reason')}")
                        continue
...
if __name__ == "__main__":
    ...
    try:
        start_time = time.time()
        logger.info("🔧 Initializing enhanced JobHunt AI Scraper V2.5...")
